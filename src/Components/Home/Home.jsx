...
// const jobEntries = [
//     {
//         company: jobs.company || "Acme Corp",
//         position: jobs.position || "Frontend Developer",
//         date: jobs.date || "2024-06-01",
//         status: jobs.status || "Interviewing"
//     }
// ];
    ...
    <div className={`flex min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-50 via-white to-pink-50'}`}>
        ...
        <div className="flex-1 flex flex-col transition-colors duration-300">
            ...
            <main className="ml-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {jobs.map((job, idx) => (
                            <div key={idx} className={`border rounded-xl p-5 shadow transition-all duration-300 hover:shadow-lg flex flex-col gap-2 ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-gradient-to-br from-white via-blue-50 to-pink-50 border-blue-100'}`}>
                                ...
                                <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-blue-400'}`}>Salary: {job.salary}</div>
                                <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-blue-400'}`}>Date: {job.date}</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className={`border mt-14 p-6 rounded-lg shadow-md transition-colors duration-300 ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-blue-200 bg-gradient-to-r from-white via-blue-50 to-pink-50'}`}>
                    <h2 className={`font-bold text-2xl mb-4 ${darkMode ? 'text-white' : 'text-blue-900'}`}>Quick Actions</h2>
                    <div className="flex flex-wrap gap-3">
                       {/* <button onClick={onUpdate} className="flex items-center gap-2 border bg-blue-600 hover:bg-blue-700 border-blue-300 dark:border-gray-700 rounded px-4 py-2 text-white transition-all duration-200 shadow font-semibold">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
